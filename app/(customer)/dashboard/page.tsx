...
function SubscriptionBadge({ days }: { days: number }) {
  if (days <= 0) {
    return (
          ...
          <div className="space-y-4">
            {devices.map((device) => {
              ...
              return (
                <div
                  key={device.id}
                  className="bg-panel border border-subtle rounded-2xl p-5 shadow-sm"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2.5 flex-wrap">
                        <h2 className="text-base font-semibold text-ink">{device.device_name}</h2>
                        <SubscriptionBadge days={days} />
                      </div>
                    ...
                    <div className="shrink-0">
                      {device.dashboard_url && !expired ? (
                        ...
                        <span
                          ...
                          title={expired ? "Subscription expired" : "No dashboard URL configured"}
                        >
                          {expired ? "Expired" : "No URL"}
                        </span>
                      )}
                    </div>
                  </div>
